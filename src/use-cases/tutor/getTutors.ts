...
export class GetAllTutorsUseCase {
  ...
  async execute(page: number, numberOfItems: number): Promise<dataGetAllTutor> {
    const tutors = await this.tutorsRepository.getAllTutors(
      page,
      numberOfItems
    );
...
export class SearchPhoneTutorUseCase {
  ...
  async execute(query: string, page: number): Promise<Tutor[]> {
...
export class SearchTutorByNameUseCase {
  ...
  async execute(query: string): Promise<Tutor[]> {
    const tutors = await this.tutorsRepository.searchByNameTutor(query);
