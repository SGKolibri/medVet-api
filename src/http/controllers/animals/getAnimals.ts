...
export async function getAllAnimals(
  ...
  try {
    ...
    const getAnimalsUseCase = new GetAllAnimalsUseCase(
...
export async function getAnimalsByTutor(
  ...
  try {
    const getusecase = makeGetAnimalByNameTutor();
...
export async function getAnimalBySequence(
  request: FastifyRequest<{ Params: Params }>,
  reply: FastifyReply
) {
...
export async function getAnimalByNameTutor(
  request: FastifyRequest<{ Params: Params }>,
  reply: FastifyReply
) {
...
export async function searchAnimalByNameOrSequnce(
  request: FastifyRequest,
  reply: FastifyReply
) {
  ...
  try {
    const { q, page } = searchParmsSchema.parse(request.query);

    const useCase = makeSearchAnimalByTutorBySequnce();
    const data = await useCase.execute(q, page);
